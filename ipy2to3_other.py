...
def convert_ipy2to3(json_to_convert):
    ...
    worksheet=json_to_convert
    if True:
        code_cells = filter(is_python_code_cell, worksheet['cells'])
        for cell in code_cells:
            magic = replace_magic_lines(cell['source'])
            file_name = None
            with tempfile.NamedTemporaryFile(
                    mode = "w", delete = False) as ostream:
                ostream.writelines(cell['source'])
                file_name = ostream.name
            cmd2to3 = ["C:/Anaconda3_64/Scripts/2to3.exe", "--nobackups", "--write", file_name]
